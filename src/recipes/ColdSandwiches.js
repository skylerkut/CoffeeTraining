...
function ColdSandwichesPage() {
    const { sandwichId } = useParams();
    const coldSandwiches = recipesData.ColdSandwiches;

// Render a single sandwich detail
const renderSandwichDetail = (id) => {
  const sandwich = coldSandwiches.find(s => s.id === id);
  ...
  return (
    ...
    const renderSandwichList = () => {
        const rows = [];
        for (let i = 0; i < coldSandwiches.length; i += 3) {
            const row = (
                <div className="row mb-2" key={`row-${i}`}>
                    {coldSandwiches.slice(i, i + 3).map((sandwich) => (
                        <div key={sandwich.id} className="col-md-4">
                            <div className="card mb-4 shadow-sm">
                                <img src={sandwich.imageUrl} alt={sandwich.title} className="img-fluid" />
                                <div className="card-body">
                                    ...
                                    <Link to={`/recipes/coldsandwiches/${sandwich.id}`}>View {sandwich.title}</Link>
                                </div>
                            </div>
                        </div>
                    ))}
            ...
            <div>
                ...
                <div className="p-4 p-md-5 mb-4 rounded text-body-emphasis bg-body-secondary">
                    <div className="col-lg-6 px-0">
                        <h1 className="display-4 fst-italic">Cold Sandwiches</h1>
  ...
  return (
    <div>
      ...
      <main className="container">
        ...
        {sandwichId ? renderSandwichDetail(sandwichId) : renderSandwichList()}
      </main>
...
export default ColdSandwichesPage;
